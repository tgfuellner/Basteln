/* Name: main.c
 * Project: Relaiscard based on PowerSwitch based on AVR USB driver
 * Hardware Ulrich Radig
 * Author: Christian Starkjohann, Thomas Gfüllner
...
#define RELAIS_COUNT 6

/*
This module implements an 6 bit parallel output controlled via USB. It is
...
/* This is the ATMEGA8 version of the routine. Change for others. */
...
static uchar    replyBuf[2];
    ...
    if(rq->bRequest == 4){  /* GET_INPUT -> result = 1 bytes */
        replyBuf[0] = PIND & 0x3;  // PD0 and PD1
        replyBuf[0] |= (PIND & ((1<<3)|(1<<4)))>>1;  // PD3 and PD4
        return 1;
    }
...
uchar   i;
    ...
    wdt_enable(WDTO_1S);
    odDebugInit();
    DDRD = ~( (1<<2)|(1<<0)|(1<<1)|(1<<3)|(1<<4) );/* all outputs except PD2 = INT0 and 4 input lines */
    DDRC |= 0xFF;
    PORTD = 0;
    PORTC = 0;

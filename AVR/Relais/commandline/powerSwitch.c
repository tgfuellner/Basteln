...
static void usage(char *name)
{
    ...
    fprintf(stderr, "Ports are single digits in the range 0...5\n");
...
int                 nBytes;
    ...
    usb_init();
    if(usbOpenDevice(&handle, USBDEV_SHARED_VENDOR, "www.obdev.at", USBDEV_SHARED_PRODUCT, "RelaisCard") != 0){
        fprintf(stderr, "Could not find USB device \"RelaisKarte\" with vid=0x%x pid=0x%x\n", USBDEV_SHARED_VENDOR, USBDEV_SHARED_PRODUCT);
        exit(1);
    }
 ...
 */
        ...
        for(i=0;i<NUMBER_OF_RELAIS;i++){
            int isOn = buffer[0] & (1 << i);
            int isInv = buffer[1] & (1 << i);
            printf("port %d: %s%s\n", i, isOn ? "on" : "off", isInv ? (isOn ? " / pulse off" : " / pulse on") : "");
        }
    }else if(strcmp(argv[1], "in") == 0){
        nBytes = usb_control_msg(handle, USB_TYPE_VENDOR | USB_RECIP_DEVICE | USB_ENDPOINT_IN, PSCMD_IN, 0, 0, (char *)buffer, sizeof(buffer), 5000);
        if(nBytes < 1){
            if(nBytes < 0)
                fprintf(stderr, "USB error: %s\n", usb_strerror());
            fprintf(stderr, "only %d bytes status received\n", nBytes);
        ...
        port = atoi(argv[2]);
        if (port >= NUMBER_OF_RELAIS || port < 0) {
                fprintf(stderr, "Sorry, there are only %d Relays\n", NUMBER_OF_RELAIS);
                exit(1);
        }

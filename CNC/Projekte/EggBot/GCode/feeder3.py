...
if options.wantDisplaceCorrection:
    pattern = re.compile(rb'([(!;].*|\s+|[a-zA-Z0-9_:](?:[+-])?\d*(?:\.\d*)?|\w\#\d+|\(.*?\)|\#\d+\=(?:[+-])?\d*(?:\.\d*)?)')
...
fileToFeed = args[0]
gcode = open(fileToFeed, "r", encoding="ascii", errors="surrogateescape")
...
for line in lines:
    ...
    print(line, "({0:.1f}%)".format((currentLine / totalLines)*100), end=' ')

    penChange(line)

    if options.wantDisplaceCorrection:
        line = correctDisplacement(line.encode())
        print(">> ", line, end=' ')
